...
const IntermediateCodeGeneration: React.FC = () => {
  ...
  const [astWidth, setAstWidth] = useState(50); // Default 50% width
  ...
  useEffect(() => {
    const fetchIntermediateCode = async () => {
      if (!abstractSyntaxTree || intermediateCode) return;
      setLoading(true);
      setErrors([]);
      setNotes([]);

      try {
        const requestBody = { ast: abstractSyntaxTree };
        console.log("Sending request to ir-generation API:", requestBody);

        const response = await fetch(`${API_BASE_URL}/api/compilation/ir-generation`, {
          method: "POST",
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch intermediate code: ${response.status} - ${errorText}`);
        }

        const data = await response.json();
        const ir = data.ir as string;
        setIntermediateCode(ir);
        setErrors(data.errors || []);
        console.log("Intermediate Code Fetched:", ir);
      } catch (error) {
        console.error("Error fetching intermediate code:", error);
        setErrors([(error as Error).message || "Failed to fetch intermediate code. Please try again."]);
      } finally {
        setLoading(false);
      }
    };

    fetchIntermediateCode();
  }, [abstractSyntaxTree, intermediateCode, setIntermediateCode]);
  ...
  const renderTree = (svgRef: React.RefObject<SVGSVGElement>, treeData: ParseTreeNode | null) => {
    ...
    const treemap = d3.tree<D3TreeNode>().nodeSize([100, 100]);
    ...
    svg
      ...
      .attr("viewBox", `${minX - 50} ${minY - 50} ${treeWidth + 200} ${treeHeight + 100}`);
        ...
        <div className="row mx-0 mt-3 d-flex justify-content-center align-items-center">
          <div className="col text-center">
            <Link
              ...
              role="button"
              aria-label="Navigate to Previous Page (Semantic Analysis)"
              tabIndex={0}
              onKeyDown={(e) => {
            ...
            <Link
              ...
              role="button"
              aria-label="Navigate to Next Page (Intermediate Code Optimization)"
              tabIndex={0}
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
