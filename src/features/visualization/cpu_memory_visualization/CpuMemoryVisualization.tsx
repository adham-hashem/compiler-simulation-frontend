...
import './CpuMemoryVisualization.css';

const CpuMemoryVisualization: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log('Loading video: CPU Memory'); // Debug log
  ...
  return (
    <div className="cpu-memory-visualization-container">
      ...
      <div className={`video-container ${isPopupOpen ? 'blurred' : ''}`}>
        <video
          ...
          muted
          playsInline
          className="cpu-memory-video"
          aria-label="CPU Memory Visualization Video"
        >
          <source src="/assets/cpu visualization.mp4" type="video/mp4" />
          Your browser does not support the video tag. Please download the video
          from <a href="/assets/cpu visualization.mp4">here</a>.
        </video>
      </div>
      ...
      {isPopupOpen && (
        <div className="popup-overlay">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="CPU Memory Concepts Information"
          >
            <h2>CPU Memory Concepts</h2>
            <p>
              CPU memory management is essential for efficient program execution and involves key concepts:
            </p>
            <ul>
              <li><strong>Registers</strong>: Fast storage within the CPU for immediate data access.</li>
              <li><strong>Cache</strong>: Small, fast memory for frequently accessed data, reducing main memory access time.</li>
              <li><strong>Main Memory (RAM)</strong>: Stores active program data and instructions for CPU processing.</li>
              <li><strong>Virtual Memory</strong>: Abstracts physical memory, allowing efficient memory allocation and isolation.</li>
            </ul>
            <button
      ...
      <div className="sliding-nav" ref={navRef}>
        ...
        <nav ref={listRef} className={`nav-list ${isMenuOpen ? 'nav-list--open' : ''}`} role="navigation" aria-label="Visualization Navigation">
          ...
          <Link
            ...
            className={`nav-item ${location.pathname === '/cpu-memory-visualization' ? 'nav-item--active' : ''}`}
            aria-label="Navigate to CPU Memory Visualization"
            aria-current={location.pathname === '/cpu-memory-visualization' ? 'page' : undefined}
            onClick={() => {
              console.log('CPU Memory clicked'); // Debug log
              setIsMenuOpen(false);
            }}
          >
...
export default CpuMemoryVisualization;